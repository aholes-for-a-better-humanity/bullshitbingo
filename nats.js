...
class ServiceImpl extends QueuedIteratorImpl {
    nc;
    _id;
    config;
    handlers;
    ...
    setupHandler(h, internal = false) {
        ...
        const callback = handler ? (err, msg)=>{
            ...
            try {
                ...
                sv.stats.countError(err1);
                msg?.respond(Empty, {
                    headers: this.errorToHeader(err1)
                });
            } finally{
    ...
    _addEndpoint(e, main = false) {
        const qi = main ? this : new QueuedIteratorImpl();
...
class DenoBuffer {
    ...
    read(p) {
        ...
        const nread = copy1(this._buf.subarray(this._off), p);
        this._off += nread;
        return nread;
    }
    writeByte(n) {
    ...
    _grow(n) {
        ...
        if (n <= Math.floor(c / 2) - m) {
            copy1(this._buf.subarray(this._off), this._buf);
        } else if (c + n > MAX_SIZE) {
            throw new Error("The buffer cannot be grown beyond the maximum size.");
        } else {
            const buf = new Uint8Array(Math.min(2 * c + n, MAX_SIZE));
            copy1(this._buf.subarray(this._off), buf);
            this._buf = buf;
        }
        this._off = 0;
...
class ServiceClientImpl {
    ...
    schema(name = "", id = "") {
        return this.q(ServiceVerb.SCHEMA, name, id);
    }
    info(name = "", id = "") {
    ...
    async info(name, data) {
        ...
        if (total && total > have) {
            ...
            while(total > have){
                ...
                const r1 = await this._request(subj, paged);
                total = r1.total;
                infos.push(r1);
                const count = Object.getOwnPropertyNames(r1.state.subjects).length;
                have += count;
                if (count < limit) {
                    break;
                }
            ...
            let subjects = {};
            for(let i1 = 0; i1 < infos.length; i1++){
                si = infos[i1];
...
class ConsumerAPIImpl extends BaseApiClient {
    ...
    async add(stream, cfg) {
        ...
        const nci = this.nc;
        const { min , ok: newAPI  } = nci.features.get(Feature.JS_NEW_CONSUMER_CREATE_API);
...
async function write(frame, writer) {
...
class DenoTransport {
    ...
    async connect(hp, options) {
        ...
        try {
            ...
            this.conn?.close();
            throw err.name === "ConnectionRefused" ? NatsError.errorForCode(ErrorCode.ConnectionRefused) : err;
        }
    }
    get isClosed() {
        return this.done;
    ...
    async startTLS(hostname) {
        const tls = this.options && this.options.tls ? this.options.tls : {};
        ...
        this.conn = await Deno.startTls(this.conn, sto);
    ...
    async _closed(err, internal = true) {
        ...
        if (!err && internal) {
            try {
                ...
                if (this.options.debug) {
                    console.log("transport close terminated with an error", err1);
                }
            }
        }
        try {
